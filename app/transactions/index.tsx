    ...
    <>
      ...
      <FlashList
        ...
        renderItem={({ item }) => {
          if (item.type === 'header') {
            return (
              <View className="flex-row justify-between border-b border-slate-200 bg-slate-100 px-4 py-2 dark:border-slate-800 dark:bg-slate-900">
              ...
              <Pressable
                ...
                <View className="flex-row border-b border-slate-200 bg-white px-4 py-3 dark:border-slate-800 dark:bg-slate-900">
                  ...
                  <View className="flex-1">
                    ...
                    <Text className="text-sm text-slate-500">{formatDate(transaction.date)}</Text>

                    <View className="mt-1 flex-row flex-wrap gap-1">
                      <View className="flex-row items-center rounded-full border border-slate-200 bg-white px-2 py-0.5 dark:border-slate-800 dark:bg-slate-900">
                        <CategoryIcon size={12} color={categoryConfig.color} />
                        <Text className="ml-1 text-xs text-slate-600 dark:text-slate-300">
                          {categoryConfig.label}
                        </Text>
                      </View>

                      {transaction.subscriptionId && (
                        <View className="flex-row items-center rounded-full border border-indigo-200 bg-indigo-50 px-2 py-0.5 dark:border-indigo-700 dark:bg-indigo-950">
        ...
        ListEmptyComponent={
          <View className="flex-1 items-center justify-center py-20">
            <Text className="text-slate-500">No transactions imported yet</Text>
