...
export const detectedSubscriptionSchema = z.object({
  name: z.string().describe("Subscription service name (e.g., 'Netflix', 'Spotify')"),
  subscribedAt: z.string().describe("ISO 8601 date of first payment detected (e.g., '2024-01-15')"),
  price: z.int().positive().describe('Price in cents'),
  domain: z.string().optional().describe("Domain of the service (e.g., 'netflix.com')"),
  billingCycle: z.enum(BILLING_CYCLES).describe('Billing cycle: weekly, monthly, or yearly'),
  confidence: z
    .number()
    .min(0)
    .max(1)
...
export const subscriptionDetectionResponseSchema = z.object({
  subscriptions: z
    ...
    .describe('Array of detected subscriptions with confidence scores and details'),
});

export type DetectedSubscription = z.infer<typeof detectedSubscriptionSchema>;

export type SubscriptionDetectionResponse = z.infer<typeof subscriptionDetectionResponseSchema>;
...
export const enrichedTransactionSchema = z.object({
  ...
  category: z.enum(CATEGORIES).describe('Transaction category based on merchant/purpose'),
  displayName: z.string().describe("Clean, human-readable name (e.g., 'Netflix', 'Migros Zurich')"),
  domain: z.string().optional().describe("Merchant domain if identifiable (e.g., 'netflix.com')"),
  subscriptionId: z
    .number()
    .optional()
    .describe('ID of matched subscription from the provided list, if applicable'),
});
...
export type TransactionEnrichmentResponse = z.infer<typeof transactionEnrichmentResponseSchema>;
